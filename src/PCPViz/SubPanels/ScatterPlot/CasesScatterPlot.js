...
const CasesScatterPlot = (props) => {
    ...
    useEffect(() => {
        ...
        const myNode = document.getElementById("cases-sub-scatterplot-vis");
        if(myNode.firstChild)
            myNode.removeChild(myNode.firstChild);

        // append the svg object to the body of the page
        var svg = d3.select("#cases-sub-scatterplot-vis")
            .append("svg")
        ...
        if(hor.horizon !== 'None') {
            const text = d3.select("#cases-sub-scatterplot-vis").select("svg").append("text")
                text.attr("transform",
        ...
        if(ver.vert !== 'None') {
            d3.select("#cases-sub-scatterplot-vis").select("svg").append("text")
